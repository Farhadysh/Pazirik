...
@section('content')
    ...
    @can('show-costs')
        ...
        <div class="col-md-12 text-center text-dark">
            <h4 class="font-weight-bold my-4 text-muted">لیست هزینه ها</h4>
        </div>
        <div class="col-md-10 mx-auto text-left">
            <a href="{{route('admin.costs.costExcel')}}" class="btn btn-outline-info cursor-pointer btn-sm">دریافت فایل اکسل</a>
        </div>
            ...
            <table class="table table-bordered rounded mt-2">
                ...
                @foreach($costs as $cost)
                    <tr class="text-center">
                        <td>{{$cost->name}}</td>
                        <td>{{$cost->count}}</td>
                        <td>{{number_format($cost->price)}}</td>
                        <td>{{$cost->date}}</td>
                        <td>{{number_format($cost->price * $cost->count)}}</td>
                        <td>{{$cost->description}}</td>
                        <td>
                            <form class="form_destroy" data-id="{{$cost->id}}" data-name="costs">
                                @csrf
                                    <a href="{{route('admin.costs.edit',['id'=>$cost->id])}}"
                                       class="btn btn-sm btn-outline-info fa fa-eye"
                                       data-toggle="tooltip"  data-placement="right" title="مشاهده هزینه"></a>
                                @can('delete-costs')
                                    <button type="submit"
