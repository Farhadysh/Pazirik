...
@section('content')
    ...
    @can('add-costs')
        <div class="col-md-12 text-center text-dark">
            <h4 class="font-weight-bold my-4 text-muted">ویرایش دفتر یا کارخانه</h4>
        </div>
        <div class="col-md-10 shadow-sm bg-white mx-auto rounded d-flex justify-content-center">
            <form action="{{route('admin.offices.update',['id'=>$office->id])}}" method="post"
                ...
                <div class="form-group col-md-3">
                    ...
                    <input type="text" class="form-control font-small" id="office_name" name="office_name"
                           placeholder="نام دفتر" value="{{$office->office_name}}">
                ...
                <div class="form-group col-md-3">
                    ...
                    <select class="form-control" name="user">
                        @foreach($users as $user)
                            <option value="{{$user->id}}" {{$user->id == $office->user_id ? 'selected' : ''}}>{{$user->name}} {{$user->last_name}}</option>
                        @endforeach
                    </select>
                    @if ($errors->has('user'))
                ...
                <div class="form-group col-md-3">
                    ...
                    <input type="text" class="form-control font-small" id="phone" name="phone"
                           placeholder="شماره تلفن دفتر" value="{{$office->phone}}">
                    @if ($errors->has('phone'))
                ...
                <div class="form-group col-md-3">
                    ...
                    <textarea type="text" class="form-control font-small" id="address" name="address"
                              placeholder="آدرس">{{$office->address}}</textarea>
                    @if ($errors->has('address'))
                        <strong class="error">{{$errors->first('address')}}</strong>
                    @endif
                </div>
                <div class="form-group col-md-3 mt-1">
                    <button type="submit" class="btn btn-outline-info btn-sm mt-2">ویرایش</button>
                </div>
            </form>
        </div>
    @endcan
